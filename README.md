...
# [Как использовать Docker по максимуму](https://12factor.net/)

- Одна кодовая база для всех сред
- Среда должна иметь свою конфигурацию для запуска приложения
- Развертывание разделено на этапы: сборка, релиз(сборка+конфигурация), выполнение
- Не сохранять внутреннее состояние (stateless) и не имеют разделяемых данных (share-nothing)
- Приложение является полностью самодостаточным и не полагается на инъекцию веб-сервера во время выполнения для того, чтобы создать веб-сервис
- Процессы приложения должны быть утилизируемыми, это означает, что они могут быть запущены и остановлены в любой момент
